...
def main():
    ...
    n_tweets = int(input('How many tweets: '))
    print("---------------------------------------------------------------")
    print(f"[INFO]: Got it! Scraping @{username_input}'s last {n_tweets} tweets.")
    tweet_df = scrape_tweets(username_input, tweets_to_get=n_tweets)
    plot_graphs(tweet_df, username_input, n_tweets)
...
def scrape_tweets(username_input, tweets_to_get=500):
    id_to_search = username_input
    tweets_to_get = tweets_to_get
...
def plot_graphs(tweet_df, username_input, n_tweets):
    print("[INFO]: Plotting graphs...")
    id_to_search = username_input
    tweets_to_get = n_tweets
    
    colors1 = ['#ff9999','#66b3ff']

    fig = plt.figure(figsize=(8,8))
    fig.suptitle(f"@{id_to_search}'s last {n_tweets} tweets summary",
                 fontweight='bold',
                 fontsize=13)
